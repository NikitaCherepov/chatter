...
app.post('/api/v1/memory/personas', (req: AuthedRequest, res: any) => {
  try {
    const persona = createPersona(
      ...
      req.body?.allow_core_memory_update !== false,
    );
...
app.patch('/api/v1/memory/personas/:personaId', (req: AuthedRequest, res: any) => {
  try {
    const persona = updatePersona(accountIdFromRequest(req), Number(req.params.personaId), {
      ...
      ...(typeof req.body?.allow_core_memory_update === 'boolean'
        ? { allow_core_memory_update: req.body.allow_core_memory_update ? 1 : 0 }
        : {}),
    });
...
app.post('/api/v1/memory/spaces/:spaceId/activate', (req: AuthedRequest, res: any) => {
  try {
...
app.patch('/api/v1/chats/:chatId/memory-records/:recordId', async (req: AuthedRequest, res: any) => {
  try {
    requireChatMemoryRecord(accountIdFromRequest(req), Number(req.params.chatId), req.params.recordId);
    const record = await VectorMemoryService.updateRecord(
      accountIdFromRequest(req), req.params.recordId,
...
app.delete('/api/v1/chats/:chatId/memory-records/:recordId', async (req: AuthedRequest, res: any) => {
  try {
    requireChatMemoryRecord(accountIdFromRequest(req), Number(req.params.chatId), req.params.recordId);
...
app.patch('/api/v1/memory/records/:recordId', async (req: AuthedRequest, res: any) => {
  try {
    requireGeneralMemoryRecord(accountIdFromRequest(req), req.params.recordId);
    const record = await VectorMemoryService.updateRecord(
      accountIdFromRequest(req),
      req.params.recordId,
      typeof req.body?.text === 'string' ? req.body.text : '',
...
app.get('/api/v1/chats', (req: AuthedRequest, res) => {
  ...
  if (folderQuery === 'unfiled') {
    folderId = null;
  } else if (folderQuery !== undefined) {
    const parsedFolderId = Number.parseInt(`${folderQuery}`, 10);
    if (!Number.isSafeInteger(parsedFolderId) || parsedFolderId <= 0) {
      return res.status(400).json({ error: 'bad_folder_id' });
    }
  ...
  const chats = listUserChats(userId, limit, offset, filters);
  const total = countUserChats(userId, filters);
  const activeChatId = ensureActiveChat(userId);
  const safeLimit = Math.max(1, Math.min(100, Number.isFinite(limit) ? limit : 50));
  const safeOffset = Math.max(0, Number.isFinite(offset) ? offset : 0);
  res.json({ chats, active_chat_id: activeChatId, total, limit: safeLimit, offset: safeOffset });
});

app.get('/api/v1/chat-filter-options', (req: AuthedRequest, res) => {
  const userId = accountIdFromRequest(req);
  return res.json(listChatFilterOptions(userId));
});
...
app.get('/api/v1/chat-folders', (req: AuthedRequest, res) => {
  ...
  return res.json({ ...listChatFolders(userId, {
    ...
    hasFiles: req.query.has_files === '1',
    hasImages: req.query.has_images === '1',
  }), active_chat_id: activeChatId, active_chat: getUserChatListItem(userId, activeChatId) || null });
});

app.post('/api/v1/chat-folders', (req: AuthedRequest, res) => {
  const userId = accountIdFromRequest(req);
  const name = `${req.body?.name || ''}`.trim();
  if (!name) return res.status(400).json({ error: 'name_required' });
  return res.status(201).json({ folder: createChatFolder(userId, name) });
});

app.put('/api/v1/chat-folders/:id', (req: AuthedRequest, res) => {
  const userId = accountIdFromRequest(req);
  const folderId = Number.parseInt(req.params.id, 10);
  if (!Number.isFinite(folderId) || folderId <= 0) return res.status(400).json({ error: 'bad_folder_id' });
  const name = `${req.body?.name || ''}`.trim();
  if (!name) return res.status(400).json({ error: 'name_required' });
  if (!renameChatFolder(userId, folderId, name)) return res.status(404).json({ error: 'folder_not_found' });
  return res.json({ ok: true });
});

app.delete('/api/v1/chat-folders/:id', (req: AuthedRequest, res) => {
  const userId = accountIdFromRequest(req);
  const folderId = Number.parseInt(req.params.id, 10);
  if (!Number.isFinite(folderId) || folderId <= 0) return res.status(400).json({ error: 'bad_folder_id' });
  if (!deleteChatFolder(userId, folderId)) return res.status(404).json({ error: 'folder_not_found' });
  return res.json({ ok: true });
});
...
app.put('/api/v1/chats/:id/folder', (req: AuthedRequest, res) => {
  const userId = accountIdFromRequest(req);
  const chatId = Number.parseInt(req.params.id, 10);
  if (!Number.isFinite(chatId) || chatId <= 0) return res.status(400).json({ error: 'bad_chat_id' });
  const rawFolderId = req.body?.folder_id;
  const folderId = rawFolderId === null || rawFolderId === undefined ? null : Number.parseInt(`${rawFolderId}`, 10);
  if (folderId !== null && (!Number.isFinite(folderId) || folderId <= 0)) {
    return res.status(400).json({ error: 'bad_folder_id' });
  }
  if (!moveUserChatToFolder(userId, chatId, folderId)) return res.status(404).json({ error: 'chat_or_folder_not_found' });
  return res.json({ ok: true, folder_id: folderId });
});
...
app.get('/api/v1/tasks/options', (req: AuthedRequest, res) => {
  const userId = accountIdFromRequest(req);
  const activeChatId = ensureActiveChat(userId);
  return res.json({
    active_chat_id: activeChatId,
    chats: listTaskTargetChats(userId),
    tools: Array.from(new Set(getTaskAllowedToolNames())).sort((a, b) => a.localeCompare(b)),
  });
});
