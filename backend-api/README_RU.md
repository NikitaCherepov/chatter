...
- `MODELS_MANUAL` - список моделей для ручного выбора. Формат: `base_url|api_key|api_model_name|display_name|description|unique_id|supports_vision|admin_only;...`
  ...
  - `admin_only` — опционально, `1`/`true` или `0`/`false` (по умолчанию `0`). Такие модели скрыты и недоступны не-админам
...
- Активный векторный движок (`qdrant` или `pinecone`) хранится в таблице `system_settings` под ключом `vector_memory_settings`. Новые установки без настроенного Pinecone используют встроенный Qdrant; существующие установки остаются на Pinecone до подтверждённой миграции.
- SQLite остаётся каноническим каталогом текста воспоминаний, владельцев, персон и пространств. В Qdrant или Pinecone хранится выбранный векторный индекс для поиска и новых записей.
- `POST /internal/admin/vector-memory/migrate-to-qdrant` переносит готовые векторы из Pinecone без повторной генерации embeddings, проверяет каждую точку и переключает движок только после успеха. Pinecone не очищается.
...
### Встроенный браузер (`browser_control`)

`browser_control` позволяет модели работать с браузером внутри Chatter Desktop. Desktop-клиент должен быть подключён; запросы из Desktop или привязанного Telegram-аккаунта передаются ему через WebSocket IPC.

- Действия: `open`, `read`, `back`, `forward`, `reload`, `scroll`, `click` и `fill`.
- `read` возвращает текст страницы и временные ссылки на элементы в режимах `viewport`, `delta` или `full`. Перед возвратом модели результат оборачивается в `<untrusted_web_content>`.
- `open`, `click` и `fill` используют настройки подтверждений пользователя (безопасное значение по умолчанию — спрашивать). Подтверждённые нажатия и ввод можно разрешить для текущего сайта до закрытия Chatter.
- Загрузки перехватываются независимо от действия, которое их запустило. Файл ожидает подтверждения до пяти минут, после чего сохраняется по выбранному пути или в папку «Загрузки»; отказ или истечение времени отменяет загрузку.
- Модель не может читать и заполнять пароли и другие чувствительные поля. Обычные видимые текстовые поля и черновики могут попасть в результат чтения страницы.
- `disable_pc_control_full` или `disable_internet` полностью убирает инструмент из доступного модели набора.

Backend не получает cookie-хранилище или локальное хранилище сайтов. На сервер отправляется только структурированный результат запрошенного действия браузера.
