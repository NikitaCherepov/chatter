...
| `modify_pixel_art` | Редактирование существующего PNG через команды рисования (`set_pixel`, `draw_rect`, `draw_line`, `fill`). Модель читает PNG через `read_pixel_art`, видит структуру, отправляет команды с координатами и цветами → бэкенд рендерит через pngjs → результат в галерею чата. Решает проблему токенизации: модель работает с координатами (числа), а не с символьными строками. |
| `read_pixel_art` | Чтение PNG файла с диска пользователя через desktop IPC → декодирует в ASCII-представление `{ width, height, palette, pixels }`. Модель видит пиксели и палитру, чтобы понять что редактировать через `modify_pixel_art`. |
...
| `disable_pc_commands` | Без команд на ПК | `execute_pc_command`, `get_file_info`, `read_file`, `search_file_keywords`, `write_file`, `edit_file_lines`, `read_pixel_art`, `modify_pixel_art` |
