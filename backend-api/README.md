...
| `read_file` | Читает файл на ПК пользователя нативно через Node.js fs (в обход терминала). Параметры: `file_path`, `start_line` (по умолчанию 1), `max_lines` (по умолчанию 500, макс. 2000). Возвращает UTF-8 контент с пагинацией. Если `file_read_enabled=true` в настройках — выполняется сразу; иначе требует HitL-карточку `file_action_confirmation`. |
