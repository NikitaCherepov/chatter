...
| `disable_pc_commands` | Без команд на ПК | `execute_pc_command`, `read_file`, `write_file`, `edit_file_lines` |
| `disable_pc_control_full` | Полная блокировка | Всё из lite + `execute_pc_command`, `read_file`, `write_file`, `edit_file_lines` + `control_smart_home`, `check_emails`, `read_email_content`, `get_my_tasks`, `explore_fs`, `desktop_action`, `map_control`, `get_map_pins`, `find_transit_route`, `search_nearby` |
