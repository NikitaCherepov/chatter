...
| `read-file` | Нативное чтение файла через `fs.createReadStream` + `readline` (UTF-8, с пагинацией). Параметры: `{ file_path, start_line?, max_lines? }`. Возвращает `{ content, start_line, read_lines, total_lines, encoding }`. Не загружает весь файл в память — построчное чтение. **Для `.docx`** использует `mammoth.extractRawText()` — извлекает чистый текст из Word-документа, затем применяет ту же пагинацию по строкам. Возвращает `format: 'docx'`. |
