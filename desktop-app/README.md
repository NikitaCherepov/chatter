...
| `suggest_chat_link` | Inline card suggesting to open a found chat (value: `{ chat_id, title }`). Rendered like `suggest_macro` — stays until user clicks "Open chat" or "Reject". Emitted automatically for the best `search_chat_history` result so the user can jump to the found conversation. |
