...
function DemoDraggableRoomMember({
  memberId,
  children,
}: {
  memberId: number;
  ...
  const {
    ...
    id: `room-member-drag-${memberId}`,
    data: { kind: 'member', memberId },
  });
  const {
    setNodeRef: setDroppableNodeRef,
    isOver,
  } = useDroppable({
    id: `room-member-drop-${memberId}`,
    data: { kind: 'member', memberId },
  });
...
export function ChatPage() {
  ...
  const [draggingRoomMemberId, setDraggingRoomMemberId] = useState<number | null>(null);
    ...
    return api.onRoomEvent((event) => {
      ...
      switch (event.type) {
        ...
        case 'chat_user_message_saved': {
          const tempId = pendingUserMessageIdByChatRef.current.get(event.chat_id);
          if (tempId === undefined) break;
          pendingUserMessageIdByChatRef.current.delete(event.chat_id);
        ...
        case 'chat_agent_start': {
          ...
          setSending(true);
          setStreamingState('idle');
          setStreamingMsgId(stream.tempId);
          break;
        }
        case 'chat_agent_token': {
          ...
          setShowTyping(false);
          setMessages((prev) => {
            const exists = prev.some((message) => message.id === stream.tempId);
            ...
            return base.map((message) => message.id === stream.tempId
        ...
        case 'chat_tool_status': {
          ...
          if (!stream) break;
          setShowTyping(false);
          const text = event.type === 'chat_tool_status' ? `_${event.text}_` : event.text;
          setMessages((prev) => {
            const exists = prev.some((message) => message.id === stream.tempId);
            const base = exists ? prev : [...prev, {
              id: stream.tempId,
        ...
        case 'chat_desktop_action': {
          if (event.action === 'execute_macro' && event.value?.commands) {
            (window as any).electronAPI?.executeCommands(event.value.commands).catch(console.error);
          }
          handleIncomingDesktopAction(event as any);
          break;
        }
        case 'chat_display_state': {
          applyAvatarState(event as any);
          break;
        }
        case 'chat_map_update': {
          openTool('map');
          dispatchMapData(event as any);
          break;
        }
        case 'chat_dice_roll': {
          finishDiceRoll(event.roll);
          break;
        }
        ...
        case 'chat_agent_error': {
          ...
          toast.error(event.message ?? t('chat.room.agentError', { error: event.error }));
          break;
        }
  ...
  const handleSend = useCallback(async () => {
    ...
    // Unified trigger: rooms persist user_only; single chat runs a default step.
    ...
    if (activeChatId) {
      ...
      const delivered = await api.sendChatTrigger({
        ...
        displayManifest: getAvatarManifest(),
        currentDisplayState: currentAvatarStateRef.current,
        isVoice,
        preferredModel,
        dice_mode: diceMode,
  ...
  const handleRoomParticipantDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    setDraggingRoomMemberId(null);
    setDraggingRoomParticipantSize(null);
    if (!activeChatId) return;
    const chatId = activeChatId;

    // Dragging a MEMBER (person) — primary queue order.
    if (active.data.current?.kind === 'member') {
      const activeId = Number(active.data.current?.memberId);
      const overId = Number(over?.data.current?.memberId);
      if (!Number.isSafeInteger(activeId) || !Number.isSafeInteger(overId) || activeId === overId) return;
      const previousMembers = roomMembers;
      const fromIndex = previousMembers.findIndex((member) => member.user_id === activeId);
      const toIndex = previousMembers.findIndex((member) => member.user_id === overId);
      if (fromIndex < 0 || toIndex < 0) return;
      const next = [...previousMembers];
      const [moved] = next.splice(fromIndex, 1);
      next.splice(toIndex, 0, moved);
      setRoomMembers(next);
      void api.reorderChatMembers(chatId, next.map((member) => member.user_id))
        .then(({ room }) => {
          if (activeChatIdRef.current === chatId) applyRoom(room);
        })
        .catch((error) => {
          console.error('Failed to reorder chat members:', error);
          if (activeChatIdRef.current === chatId) {
            setRoomMembers(previousMembers);
            toast.error(t('auth.error.generic'));
          }
        });
      return;
  ...
  const toggleRoomCharacterAccess = async (participant: api.ChatAgent) => {
    if (!activeChatId || roomSaving) return;
    const chatId = activeChatId;
    const next = participant.access === 'shared' ? 'private' : 'shared';
    setRoomSaving(true);
    try {
      const { room } = await api.updateChatAgent(chatId, participant.id, { access: next });
      if (activeChatIdRef.current === chatId) applyRoom(room);
    } catch (error) {
      console.error('Failed to toggle agent access:', error);
      toast.error(t('auth.error.generic'));
    } finally {
      if (activeChatIdRef.current === chatId) setRoomSaving(false);
    }
  ...
  const isRoomAdmin = roomMembers.some((member) => member.user_id === user?.id && member.role === 'admin');
  const menuMessage = msgMenuId === null ? undefined : messages.find((message) => message.id === msgMenuId);
  const canEditMenuMessage = !menuMessage || menuMessage.user_id == null || menuMessage.user_id === user?.id;
            ...
            <div className={s.inputArea}>
              ...
              {sending ? (
                <svg
                  ...
                  onClick={() => {
                    if (activeChatId !== null) api.stopRoomStream(activeChatId);
                  }}
            ...
            <div className={s.roomPanelInner}>
              ...
              <div className={s.roomSection}>
                <div className={s.roomParticipantsHeader}>
                  ...
                  <span className={s.roomParticipantTotal}>{roomMembers.length + roomCharacters.length}</span>
                </div>
                <DndContext
                  ...
                  onDragStart={({ active }) => {
                    ...
                    setRoomParticipantMenuId(null);
                    if (kind === 'member') {
                      const memberId = Number(active.data.current?.memberId);
                      setDraggingRoomMemberId(Number.isSafeInteger(memberId) ? memberId : null);
                      setDraggingRoomParticipantId(null);
                    } else {
                ...
                >
                  <div className={s.roomParticipantList}>
                    {roomMembers.map((member) => {
                      ...
                      const memberAgents = roomCharacters.filter((participant) => participant.owner_user_id === member.user_id);
                      const displayName = isSelf
                        ? (user?.name || user?.username || t('chat.room.you'))
                        : (member.name || `#${member.user_id}`);
                      return (
                        <React.Fragment key={`member-${member.user_id}`}>
                          <DemoDraggableRoomMember memberId={member.user_id}>
                            {(dragHandleProps) => (
                              <>
                                {isRoomAdmin && roomMembers.length > 1 && (
                                ...
                                <span className={`${s.roomParticipantAvatar} ${s.roomAvatarHuman}`}>{displayName.trim().charAt(0).toUpperCase()}</span>
                          ...
                          {memberAgents.map((participant, index) => (
                                ...
                                <>
                                  ...
                                  <div className={s.roomParticipantInfo}>
                                    {renamingRoomParticipantId === participant.id ? (
                                      ...
                                      <strong>
                                        {participant.name}
                                        {participant.access === 'shared' && (
                                          <span className={s.roomSharedBadge} title={t('chat.room.sharedBadge')}>{t('chat.room.sharedBadge')}</span>
                                        )}
                                      </strong>
                                    )}
                                  ...
                                  {roomParticipantMenuId === participant.id && (
                                    <div className={s.roomParticipantContextMenu} onClick={(event) => event.stopPropagation()}>
                                      ...
                                      {participant.owner_user_id === user?.id && (
                                        <button type="button" onClick={() => void toggleRoomCharacterAccess(participant)}>
                                          ...
                                          {participant.access === 'shared' ? t('chat.room.makePrivate') : t('chat.room.makeShared')}
                                        </button>
                                      )}
                  ...
                  <DragOverlay zIndex={10002} dropAnimation={null}>
                    {draggingRoomMemberId && (() => {
                      const member = roomMembers.find((item) => item.user_id === draggingRoomMemberId);
              ...
              {roomMembers.some(m => m.user_id === user?.id && m.role === 'member') && (
                ...
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><path d="M21 12H9"/></svg>
                  {t('chat.room.leaveRoom')}
                </button>
              )}
              </>
