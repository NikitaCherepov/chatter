...
export async function getChats(limit = 25, offset = 0, filters: ChatListFilters = {}): Promise<{ chats: ChatInfo[]; active_chat_id: number | null; total: number }> {
  ...
  if (filters.folderId === null) params.set('folder', 'unfiled');
  else if (filters.folderId !== undefined) params.set('folder', `${filters.folderId}`);
...
export type DesktopActionPayload = {
  action: 'open_widget' | 'close_widget' | 'set_widget_data' | 'open_note' | 'read_widget_state' | 'toggle_panel' | 'execute_macro' | 'suggest_macro' | 'devops_confirmation' | 'suggest_devops_runbook' | 'suggest_server_creds_update' | 'pc_command_confirmation' | 'confirmation_resolved' | 'browser_action_confirmation' | 'browser_action_confirmation_resolved' | 'browser_download_confirmation' | 'browser_download_confirmation_resolved' | 'file_action_confirmation' | 'edit_file_lines_confirmation' | 'email_confirmation' | 'chat_title_update' | 'webcam_capture_confirmation' | 'suggest_chat_link';
  target?: string;
...
export type NewspaperIssueSummary = {
  id: number;
  newspaper_id: number;
  issue_number: number;
  title: string;
...
export type NewspaperIssue = NewspaperIssueSummary & { document: NewspaperIssueDocument };
...
export type Newspaper = {
  ...
  enabled: boolean;
  issue_count: number;
  latest_issue: NewspaperIssueSummary | null;
  created_at: number;
  updated_at: number;
};
...
export async function listNewspaperIssues(newspaperId: number, limit = 30): Promise<{ issues: NewspaperIssueSummary[] }> {
  return apiFetch(`/api/v1/newspapers/${newspaperId}/issues?limit=${limit}`);
}
...
export async function createDemoNewspaperIssue(): Promise<{ issue: NewspaperIssue }> {
  return apiFetch('/api/v1/newspapers/demo', { method: 'POST' });
}

export async function deleteNewspaperIssue(issueId: number): Promise<{ ok: boolean }> {
  return apiFetch(`/api/v1/newspaper-issues/${issueId}`, { method: 'DELETE' });
}
