...
export function BroadsheetTemplate({ issue, content }: NewspaperTemplateProps) {
  ...
  return <article className={`${s.page} ${s.broadsheet}`}>
    ...
    <div className={s.metroGrid}><aside className={s.metroLeft}><section><h3>{weather?.location}</h3><h2>{weather?.condition}</h2><WeatherForecast weather={weather} compact/></section>{noteLists.map(list => <section key={list.id}>{list.title && <h2>{list.title}</h2>}<NotesListBlock list={list}/></section>)}</aside>
