...
export function WizardingBodyBlock({ block, notesPage = false }: { block: NewspaperBlock; notesPage?: boolean }) {
  ...
  if (block.type === 'notes_list') {
    const isLargeList = block.items.length >= 4;
    return <section className={`${t.bodyList} ${isLargeList ? t.bodyListLarge : ''}`}>
      {block.title && <h2>{block.title}</h2>}
      <NotesListBlock list={block} className={notesPage ? t.notesColumns : isLargeList ? t.bodyListColumns : undefined}/>
    </section>;
  }
