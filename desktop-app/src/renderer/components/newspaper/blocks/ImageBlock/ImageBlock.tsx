...
export function ImageBlock({ image, className }: { image: ImageBlockData; className?: string }) {
  const url = safeImageUrl(image.image_url, 1200);
  const fullUrl = safeImageUrl(image.image_url, 1920);
  const openImage = useNewspaperImageViewer();
  if (!url) return null;
  return <figure className={className}><button className={s.imageOpen} type="button" onClick={() => openImage?.(fullUrl || url, image.title)} aria-label={`Открыть изображение: ${image.title}`}><motion.img src={url} alt="" whileHover={{ scale: 1.025 }} transition={{ duration: 0.22, ease: 'easeOut' }}/></button><figcaption><strong>{image.title}</strong>{image.caption && <span>{image.caption}</span>}</figcaption></figure>;
}
