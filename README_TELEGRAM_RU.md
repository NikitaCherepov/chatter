...
- **search_chat_history** - полнотекстовый поиск по истории чатов пользователя (FTS5). Возвращает отдельные сообщения со сниппетом, названием чата, chat_id и message_id. Чаты, скрытые через desktop (bot_hidden), исключаются. Используется для поиска прошлых обсуждений.
- **read_chat_context** - чтение окна сообщений (±n) вокруг указанного message_id в чате. Используется после search_chat_history для просмотра контекста найденного сообщения. Поддерживает пагинацию через has_more_before / has_more_after.
