...
### 2026-06-14: Архивация сообщений (soft delete)

**Проблема:** При превышении `context_window_max` старые сообщения физически удалялись из БД. Десктоп не мог загрузить старые сообщения через пагинацию.

**Решение:** `trimUserHistoryByChat()` переведён с `DELETE` на `UPDATE ... SET archived = 1`. Архивные сообщения исключаются из AI-контекста (`getHistoryForAi`), но остаются в БД и доступны в десктопе. См. [backend-api/README.md](./backend-api/README.md).

### 2025-05-03: Image Storage & Persistence

**Проблема:** Картинки (пользовательские и сгенерированные) нигде не сохранялись. После отправки фото в AI или генерации — base64 терялся, в истории чата картинки не отображались ни на desktop, ни в TG.

**Решение:**
- Бэкенд: новый сервис `image-storage.ts` (sharp) — сохраняет уменьшенные копии пользовательских фото (webp, 512px) и полноразмерные сгенерированные (PNG) в папку `uploads/`.
- БД: колонка `images TEXT` в `chat_messages` — JSON-массив с URL и типом (`user_photo` / `generated`).
- AI история **не содержит картинок** — только текст `[Фото]caption`. Токены не тратятся на повторную отправку.
- Desktop: рендеринг картинок в бабблах, модалка просмотра (85% экрана) с кнопкой скачивания, hover-кнопка скачивания на миниатюрах.
- TG: фото пользователя сохраняются как thumbnail на сервере, отображаются при загрузке истории в desktop.
