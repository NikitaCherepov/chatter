...
const processUserTextThroughAi = async (
    ...
    try {
        ...
        if (!forceProRoute) {
            ...
            const routerPrompt = `Ты — маршрутизатор запросов. Твоя цель — определить категорию запроса. Если ты НЕ можешь определить тип запроса, или он выбивается из твоих доступных категорий, перенаправляй его в PRO.
