...
const processUserTextThroughAi = async (
    ...
    try {
        ...
        if (!forceProRoute) {
            ...
            const hasExplicitCheapIntent = /\b(включи|выключи|переключи|запусти|останови|найди|поищи|загугли|погугли|проверь\s+в\s+интернете|найди\s+в\s+сети|установи\s+часов(ой|ые)\s+пояс|смени\s+часов(ой|ые)\s+пояс|какой\s+часов(ой|ые)\s+пояс|брось\s+кубик|кинь\s+кубик|подбрось\s+монетку|монетк[ау]|рандом|random|roll|coin)\b/i.test(userText);
...
ВАЖНО:
1) Выбирай SMART_HOME / QUICK_SEARCH / TIMEZONE / RANDOM ТОЛЬКО если пользователь ЯВНО И ПРЯМО просит выполнить именно это действие.
2) Если запрос не является явной командой, либо это рассуждение/вопрос/болтовня/неясная формулировка — всегда выбирай PRO.
3) Если в запросе есть отложенное/регулярное действие по времени ("через ...", "завтра", "в 10:30", "напомни", "каждый день"), выбирай ТОЛЬКО PRO, даже если там есть погода/поиск.

Запрос пользователя: "${userText}"`;

            if (hasExplicitCheapIntent && !hasSchedulingIntent) {
                try {
